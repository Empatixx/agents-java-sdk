...
class RenderEventListenerTest {
    ...
    void toolCompletedCallsPrintToolResult() {
        bus.emit(new AgentEvent.ToolCompleted("read_file", "file content", 1, 50));
        assertTrue(renderer.lines().stream().anyMatch(l -> l.contains("[result] read_file")));
    }

    @Test
    void toolBlockedCallsPrintPermissionDenied() {
        bus.emit(new AgentEvent.ToolBlocked("write_file", "denied"));
    ...
    @Test
    void errorOccurred() {
        bus.emit(new AgentEvent.ErrorOccurred("something broke"));
        assertTrue(renderer.errors().contains("something broke"));
    }

    @Test
    void taskNotificationRendered() {
        bus.emit(new AgentEvent.TaskNotification("t-1", "agent-1", "COMPLETED", "found stuff"));
    ...
    void mailboxMessageRendered() {
        bus.emit(new AgentEvent.MailboxMessage("agent-2", "hello from agent"));
    ...
    void sessionLoadedRendered() {
        bus.emit(new AgentEvent.SessionLoaded("abc123", 42));
    ...
    void userEventsDoNotCrashRenderer() {
        ...
        assertDoesNotThrow(() -> {
            bus.emit(new AgentEvent.UserPromptSubmitted("hello"));
            bus.emit(new AgentEvent.UserSelectionMade("ctx", 0, "Yes"));
            bus.emit(new AgentEvent.UserTextInputSubmitted("ctx", "text"));
            bus.emit(new AgentEvent.CommandExecuted("help", null));
